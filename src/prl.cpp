...
public:
    ...
    auto on_event(const MsgPdEvents& event) -> etl::fsm_state_id_t {
        ...
        if (!ams_requested) {
            prl_tx.flags.clear(PRL_TX_FLAG::WAIT_SINK_OK);
        } else {
            if (!prl_tx.flags.test(PRL_TX_FLAG::WAIT_SINK_OK)) {
                prl_tx.flags.set(PRL_TX_FLAG::WAIT_SINK_OK);
                return PRL_Tx_Snk_Start_of_AMS;
            }
        }

        // For non-AMS messages, or after SinkTxOK received for first one
