...
public:
    ...
    auto on_enter_state() -> etl::fsm_state_id_t override {
        ...
        // If sink is ready - current DPM request completed (with success or
        // failure). In edge case (AMS interrupt by discard) - it was rearmed.
        pe.dpm_current_request = 0;
...
public:
    ...
    auto on_event(__maybe_unused const MsgPdEvents& event) -> etl::fsm_state_id_t {
            ...
            pe.flags.test_and_clear(PE_FLAG::MSG_RECEIVED))
        {
            ...
            PE_LOG("Protocol error: unexpected message received [%" PRIX16 "]", hdr.raw_value);
            return PE_SNK_Send_Soft_Reset;
        }
